# Bidirecionalidade canônica por módulo — Força de Vendas

Este artefato canônico preserva a classificação corrigida e aprovada do Passo 01, sem reinterpretar operações nem promover IDs relacionais a chaves primárias.
...
Rastreabilidade intermediária: `analises/ferramentas/forca-de-vendas/01-inventario-operacoes.md`, `analises/ferramentas/forca-de-vendas/01-matriz-bidirecionalidade-modulos.md` e `parceiros/execucoes/ferramentas/forca-de-vendas/referencias/documentacao-oficial/Documentacao_API_Forca_de_Vendas.extraido.txt`.
...
## Limites operacionais transversais

| Dimensão | Registro canônico |
|---|---|
| Confirmação válida no destino | `NAO_DOCUMENTADO`; sucesso HTTP não é prova de persistência ou confirmação no outro sistema. |
| Idempotência | `NAO_DOCUMENTADO` para operações mutáveis (`PEND-003`). |
| Prevenção de loop e origem da alteração | `NAO_DOCUMENTADO` nos módulos com leitura e escrita (`PEND-004`). |
| Política de conflito | `NAO_DOCUMENTADO` nos módulos com leitura e escrita (`PEND-008`). |
| Reprocessamento seguro | `NAO_DOCUMENTADO` para operações mutáveis (`PEND-009`). |
| Rate limit, quota, timeout e retry/backoff | `NAO_DOCUMENTADO` transversalmente (`PEND-002`). |

Esses registros aplicam-se a cada módulo conforme seu escopo, ainda que não sejam repetidos em todas as seções. Eles não são preenchidos por convenção e impedem que a cobertura documental 66/24 seja interpretada como homologação operacional.
...
Conclusão: este contrato de bidirecionalidade cobre integralmente as 90 operações e as duas perspectivas exigidas, mas registra capacidade documental, não homologação operacional. `PEND-001` a `PEND-020` permanecem explícitas e o uso em ERP parceiro continua bloqueado até futura homologação válida do Passo 03.
